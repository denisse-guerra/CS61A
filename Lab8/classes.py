...
class Player:
    def __init__(self, deck, name):
        ...
        self.name = name
        # BEGIN Problem 2
        self.hand = [self.deck.draw() for i in range(5)] ###might have to switch later to class method if stuff bugs
...
class ProfessorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        >>> player1, player2 = Player(player_deck, 'p1'), Player(opponent_deck, 'p2')
        >>> opponent_card = Card("other", 500, 500)
        >>> test_card = ProfessorCard("Professor Card", 1000, 1000)
        >>> player1.hand = [Card("card1", 0, 100)]
        >>> test_card.effect(opponent_card, player1, player2)
        Professor Card returns to my hand!
        >>> print(player1.hand) # survives with non-negative attack
        [card1: Staff, [0, 100], Professor Card: Professor, [0, 0]]
        >>> player1.hand = [Card("card1", 0, 100)]
        >>> test_card.effect(opponent_card, player1, player2)
        >>> print(player1.hand)
        [card1: Staff, [0, 100]]
        ...
        # BEGIN Problem 6
        readd = None
        if self.power(opponent_card) > opponent_card.power(self):
