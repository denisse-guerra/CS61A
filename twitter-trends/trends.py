...
def extract_words(text):
    ...
    ['cat', 'on', 'my', 'keyboard']
    """
    output_words = ''.join((c if c.isalpha() else ' ') for c in text).split()
    return output_words # Replace this line
...
def analyze_tweet_sentiment(tweet):
    ...
    extracted = tweet_words(tweet)
    total_senval = 0
    count = 0
    for word in extracted:
        word_senti = get_word_sentiment(word)
        # word_senti is the sentiment object of a word in the tweet
        if has_sentiment(word_senti):
            total_senval += sentiment_value(word_senti)
            count += 1
    if count:
        return make_sentiment(total_senval/count)
    else:
...
def find_centroid(polygon):
    ...
    (1.0, 2.0, 0.0)
    """
    sum_lat, sum_lon, sum_area = 0, 0, 0
    for i in range(len(polygon)-1):
        if i == len(polygon) - 2:
            sum_lat += (latitude(polygon[i])+latitude(polygon[0]))*(
                latitude(polygon[i])*longitude(polygon[0])-longitude(polygon[i])*latitude(polygon[0]))
            sum_lon += (longitude(polygon[i])+longitude(polygon[0]))*(
                latitude(polygon[i])*longitude(polygon[0])-longitude(polygon[i])*latitude(polygon[0]))
            sum_area += latitude(polygon[i])*longitude(polygon[0])-longitude(polygon[i])*latitude(polygon[0])
        else:
            sum_lat += (latitude(polygon[i])+latitude(polygon[i+1]))*(
                latitude(polygon[i])*longitude(polygon[i+1])-longitude(polygon[i])*latitude(polygon[i+1]))
            sum_lon += (longitude(polygon[i])+longitude(polygon[i+1]))*(
                latitude(polygon[i])*longitude(polygon[i+1])-longitude(polygon[i])*latitude(polygon[i+1]))
            sum_area += latitude(polygon[i])*longitude(polygon[i+1])-longitude(polygon[i])*latitude(polygon[i+1])
    if sum_area == 0:
        return [latitude(polygon[0]), longitude(polygon[0]), 0]
    else:
        signed_area = sum_area/2
        return [sum_lat/(6*signed_area), sum_lon/(6*signed_area), abs(signed_area)]
...
def group_tweets_by_state(tweets):
    ...
    def find_nearest_state(tweet):
        ...
        for key in us_states:
            distance = geo_distance(find_state_center(us_states[key]), tweet_location(tweet))
...
def average_sentiments(tweets_by_state):
    ...
    """
    def get_state_average(state):
        total_state_sentiment = 0
        sentiment_count = 0
        for tweet in tweets_by_state[state]:
            if has_sentiment(analyze_tweet_sentiment(tweet)):
                total_state_sentiment += sentiment_value(analyze_tweet_sentiment(tweet))
                sentiment_count += 1
        if sentiment_count == 0:
            return False
        else:
            return total_state_sentiment/sentiment_count

    averaged_state_sentiments = {}
    for state in tweets_by_state:
        if get_state_average(state):
            averaged_state_sentiments[state] = get_state_average(state)
